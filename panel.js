...
async function saveSession(d) {
  ...
  await chrome.storage.local.set({ [CFG.sk]: s });
  S.auth = s;
}

async function loadStoredSession() {
  const result = await chrome.storage.local.get(CFG.sk);
  return result[CFG.sk] || null;
}

async function clearSession() {
  await chrome.storage.local.remove(CFG.sk);
}
...
async function rpc(fn, params) {
  ...
  if (!res.ok) {
    ...
    if (fn === 'get_dashboard_stats' && (msg.includes('does not exist') || msg.includes('function'))) {
      throw new Error('Função de dashboard não encontrada. Execute a migration SQL no Supabase primeiro (arquivo: supabase/migrations/20260602000000_add_dashboard_rpc.sql).');
    }
...
/* ─── SVG ICONS ───────────────────────────────────────────────────────── */
const ICON = {
  shield: (sz) => `<svg width="${sz}" height="${sz}" viewBox="0 0 48 48" fill="none" style="flex-shrink:0">
...
/* ─── RENDER ──────────────────────────────────────────────────────────── */
function render() {
  document.getElementById('app').innerHTML = S.view === 'login' ? htmlLogin() : htmlShell();
  bindAll();
}
...
<div class="card">
  ...
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr>
        <th>Login Cliente</th><th>Número da Venda</th><th>Responsável</th>
        <th>E-mail</th><th>Data / Hora</th><th>Link</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
  </div>
...
function htmlDashboard() {
  ...
  const monthOpts = Array.from({ length: 12 }, (_, i) => {
    ...
    const sel = (S.customFrom &&
      S.customFrom.getFullYear() === d.getFullYear() &&
      S.customFrom.getMonth() === d.getMonth()) ? 'selected' : '';
...
function bindAll() {
  ...
  document.querySelectorAll('[data-view]').forEach(btn => {
    btn.addEventListener('click', () => {
      if (S.view !== btn.dataset.view) { S.view = btn.dataset.view; render(); }
    });
  });
...
async function onLoginSubmit(e) {
  ...
  S.loginError = null; S.loginLoading = true;
  render();
...
async function onSearch() {
  ...
  S.searchError = null; S.searchResults = null; S.searchLoading = true;
  render();
...
async function onPeriodChange(period) {
  if (S.period === period && period !== 'custom') return;
  S.period = period;
  ...
  S.dashboardData = null; S.dashboardLoading = true; S.dashboardError = null;
  render();
  await loadDashboard();
  render();
}
...
async function onMonthChange(e) {
  ...
  S.dashboardData = null; S.dashboardLoading = true;
  render();
  await loadDashboard();
  render();
}
...
async function loadDashboard() {
  const { from, to } = getPeriodRange();
  S.dashboardLoading = true; S.dashboardError = null;
  try {
